...
```{r}
tax_assignment_table <- read_xlsx("emu_taxonomic_profile.xlsx") #loading in the taxonomic data
...
sample_table <- read_xlsx("sample_table.xlsx") #loading in the sample data
sample_table <- transform(sample_table, concentration = as.numeric(concentration)) #making the concentration column numeric which we need for the decontamination process
...
```{r}
phyloseq_data_sub <- subset_samples(phyloseq_data, sample_type =="sample" | sample_type =="n2_sample" | sample_type =="resevoir" | sample_type =="control") #subsetting the samples and the controls
...
#removing taxa that are not present in our samples (only present in controls or reservoir samples)
  ...
  for (j in c(1:(nrow(tax_table_new) - 1))){ #for each row (except for the last unspecified row)
    if (is.na(tax_table_new[j,i])){ #if the value in the specified row and column is NA
      ...
      while (is.na(tax_table_new[j,i])){ #while the value in the row/column is NA
        ...
        output <- tax_table_edit[j,k] #the new value in the cell
        k <- k - 1 #reset K
      }
